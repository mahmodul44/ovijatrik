...
<div class="p-4 max-w-7xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-900 shadow-md rounded-xl p-6 border border-gray-200 dark:border-gray-700">
        ...
        @endif
        <form id="memberreciptInsertForm" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
           ...
           <div class="flex flex-col md:flex-row gap-4 md:items-end">
                <div class="w-full md:w-2/4">
                    <label for="member_id" class="block text-gray-700 dark:text-gray-300 font-medium mb-1">Member <span class="text-red-600">*</span></label>
                    <select required id="member_id" name="member_id" 
                        class="member-select w-full px-4 py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 text-sm bg-white dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200">
                        @foreach ($memberlist as $value)
                            <option value="{{ $value->id }}" 
                                {{ $value->id == $invoiceInfo->member_id ? 'selected' : '' }}>
                                {{ $value->member_id }} - {{ $value->name }} ({{ $value->phone_no }})
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="w-full md:w-1/4">
                    <label for="payment_date" class="block text-gray-700 dark:text-gray-300 font-medium mb-1">Date <span class="text-red-600">*</span></label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <select required id="account_id" name="account_id"
                    ...
                    @foreach ($accounts as $item)
                        <option value="{{ $item->account_id }}" {{ $invoiceInfo->account_id == $item->account_id ? 'selected' : '' }}>{{ $item->account_name }} ({{ $item->account_no }})</option>
                    @endforeach
                </select>
                </div>
                <div>
...
renderFiscalMonths();
let selectedMonths = {!! json_encode(json_decode($invoiceInfo->selected_months, true)) !!};

$(document).ready(function () {
    $('#member_id').trigger('change');
    setTimeout(function () {
        if (selectedMonths && selectedMonths.length > 0) {
            selectedMonths.forEach(function (value) {
                $(`input.month-checkbox[value="${value}"]`).prop('checked', true);
            });
...
$("#memberreciptInsertForm").on('submit', function(e){
    e.preventDefault();
    let thisForm = $(this);

    $.ajax({
        type: "POST",
        url: "{{route('memberreceipt.update',$invoiceInfo->mr_id)}}",
