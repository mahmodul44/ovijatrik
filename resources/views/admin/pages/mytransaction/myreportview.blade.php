...
<head>
    ...
    <title>My Transaction Report</title>
    <style>
        ...
        @if(count($reportData) > 0)
            <table>
                ...
                <tbody>
                    @php 
                        $balance = 0; 
                        $projectMap = $reportData->pluck('project_name','project_id'); // project_id => project_name
                    @endphp
                    @foreach($reportData as $row)
                        ...
                        <tr>
                            ...
                            <td style="text-align: left">{{ $row->project_name ?? 'N/A' }}</td>
                            <td class="text-green" style="text-align: right">{{ number_format($row->total_receipt,2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
