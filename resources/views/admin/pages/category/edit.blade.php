    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <nav class="text-sm text-gray-500 dark:text-gray-400 mb-3 md:mb-0" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li class="text-gray-700 dark:text-gray-200 font-medium">Category Update</li>
            </ol>
        </nav>
            ...
            <div class="flex flex-col md:flex-row md:items-end gap-4">
                <div class="w-full">
                    ...
                    <input type="text" required name="category_name_bn" id="category_name_bn" 
                           value="{{ old('category_name_bn',$category->category_name_bn) }}"
                           class="w-full border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition"/>
                </div>
            </div>
...
$("#categoryupdateForm").on('submit', function(e){
    ...
    $.ajax({
        type: "POST",
        url: "{{route('category.update',$category->category_id)}}",
