...
<head>
    ...
    <title>Head Wise Statement - {{ $category->category_name }}</title>
    <style>
...
<div class="container">
    <div class="header">
        ...
        <div class="report-title">
            <h2>Head Wise Statement</h2>
            <p><strong>Head:</strong> {{ $category->category_name }}</p>
    ...
    <thead>
        <tr>
            ...
            <th width="20%" class="text-right">Credit (TK)</th>
            <th width="20%" class="text-right">Debit (TK)</th>
            <th width="20%" class="text-right">Net Balance (TK)</th>
        </tr>
    </thead>
    <tbody>
        @php 
        ...
        @foreach($reportData as $data)
            @php
                $netBalance = $data->total_credit - $data->total_debit;
            ...
            <tr>
                ...
                <td class="text-right deposit">{{ number_format($data->total_credit, 2) }}</td>
                <td class="text-right withdraw">{{ number_format($data->total_debit, 2) }}</td>
                <td class="text-right font-bold">{{ number_format($netBalance, 2) }}</td>
            </tr>
        @endforeach
    </tbody>
    <tfoot>
        <tr class="footer-row">
            ...
            <td class="text-right deposit">{{ number_format($grandTotalCredit, 2) }}</td>
            <td class="text-right withdraw">{{ number_format($grandTotalDebit, 2) }}</td>
            <td class="text-right balance-total">{{ number_format($grandTotalBalance, 2) }}</td>
        </tr>
    </tfoot>
</table>
