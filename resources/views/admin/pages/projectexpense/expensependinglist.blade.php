...
<div id="expenseModal" class="fixed inset-0 bg-gray-700 bg-opacity-50 hidden items-center justify-center z-50">
  <div class="bg-white rounded-lg shadow-lg max-w-lg w-full p-6">
    ...
    <div class="flex justify-between items-center border-b pb-2 border-gray-200 dark:border-gray-700">
      <h3 class="text-lg font-semibold text-gray-800">Project Expense Details</h3>
    ...
    <div class="mt-6 flex justify-end gap-3">
      <button class="p-2 bg-red-600 hover:bg-red-700 text-white rounded-full shadow" onclick="closeModal()" title="Close">
       Close
      </button>
    </div>
  </div>
...
function approveExpense(expenseId, btn) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("{{ route('projectexpense.approveprojectexpense') }}", {
                method: "POST",
                headers: {
            ...
            .catch(() => {
                toastr.error("Error approving expense.");
            });
        }
    });
}
...
function declineExpense(expenseId, btn) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch("{{ route('projectexpense.projectexpensedecline') }}", {
                method: "POST",
                headers: {
                    "X-CSRF-TOKEN": "{{ csrf_token() }}",
                    "Content-Type": "application/json"
...
function viewExpense(expenseId) {
    ...
    fetch("{{ url('admin/projectexpense/show') }}/" + expenseId)
        .then(res => res.json())
        .then(data => {
            if (data) {
                let html = `
                ...
                <table class="min-w-full border border-gray-300 divide-y divide-gray-200 text-sm text-gray-700">
                    ...
                    <tr>
                        <td class="px-4 py-2 font-semibold bg-gray-50">Project</td>
                        <td class="px-4 py-2">
                            ${data.project_id ? data.project.project_title : ''}
                        </td>
                    </tr>
                    <tr>
